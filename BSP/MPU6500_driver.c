...
void MPU6500_GetMotion6(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz)
{
  ...
  MPU6500_Lastgz = mpu_buff[12]<<8 |mpu_buff[13] - imu_data_offest.gz;
	...
	*gx = MPU6500_FIFO[3][10]  - 16;//Gx_offset   RED3: 21  0 5  R2:-34 20 -22
	*gy = MPU6500_FIFO[4][10]  - 17 ;//Gy_offset
	*gz = MPU6500_FIFO[5][10]  - 15;//Gz_offset
	gy_data=*gz;
...
void MPU6500_InitGyro_offset(void)
{
	...
	{
		delay_us(100);
		MPU6500_GetMotion6(&temp[0], &temp[1], &temp[2], &temp[3], &temp[4], &temp[5]);
	}
