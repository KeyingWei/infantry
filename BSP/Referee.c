		...
		for(i = 0; i < frame_len; i ++)
		{
			if(Rx_Buf_Referee[i] == 0xA5)
			{
				...
				if(Verify_CRC8_Check_Sum(Rx_Buf_Referee,i+5))
				{
					...
					{
				//		Referee_date1.GameRobotState_t.stageRemianTime = (Rx_Buf_Referee[i + 8] << 8)  | Rx_Buf_Referee[i+7];\
						Referee_date1.GameRobotState_t.gameProgress    =  Rx_Buf_Referee[i + 9];\
						Referee_date1.GameRobotState_t.robotLevel      =  Rx_Buf_Referee[i + 10];\
						Referee_date1.GameRobotState_t.remainHP        = (Rx_Buf_Referee[i + 12] << 8 )| Rx_Buf_Referee[i+11];\
						Referee_date1.GameRobotState_t.maxHP           = (Rx_Buf_Referee[i + 14]) << 8 | Rx_Buf_Referee[i + 13];
			...
			//			printf("CmdID=%d  stageRemianTime=%d  gameProgress=%d  robotLevel=%d  remainHP=%d   maxHP=%d \r\n",Referee_date1.CmdID,Referee_date1.GameRobotState_t.stageRemianTime,Referee_date1.GameRobotState_t.gameProgress,\
				...
				//		 printf("CmdID=%d  armorType= %d   hurtType=%d\r\n", Referee_date1.CmdID,Referee_date1.RoboHurt_t.armorType,Referee_date1.RoboHurt_t.hurtType);
					...
					{
		//				Referee_date1.ShootData.bulletType  =  Rx_Buf_Referee[i+7] ;\
						Referee_date1.ShootData.bulletFreq  =  Rx_Buf_Referee[i+8] ;\
						Referee_date1.ShootData.bulletSpeed =  (Rx_Buf_Referee[i + 12] << 24)| (Rx_Buf_Referee[i + 11] << 16) | (Rx_Buf_Referee[i + 10] << 8) | Rx_Buf_Referee[i+9];
					...
					{
//					 Referee_date1.powerHeatData.chassisVolt        = (Rx_Buf_Referee[i + 10] << 24)| (Rx_Buf_Referee[i + 9] << 16)  | (Rx_Buf_Referee[i + 8] << 8)  | Rx_Buf_Referee[i+7];\
						 Referee_date1.powerHeatData.chassisCurrent     = (Rx_Buf_Referee[i + 14] << 24)| (Rx_Buf_Referee[i + 13] << 16) | (Rx_Buf_Referee[i + 12] << 8) | Rx_Buf_Referee[i+11];\
						 Referee_date1.powerHeatData.chassisPower       = (Rx_Buf_Referee[i + 18] << 24)| (Rx_Buf_Referee[i + 17] << 16) | (Rx_Buf_Referee[i + 16] << 8) | Rx_Buf_Referee[i+15];\
						 Referee_date1.powerHeatData.chassisPowerBuffer = (Rx_Buf_Referee[i + 22] << 24)| (Rx_Buf_Referee[i + 21] << 16) | (Rx_Buf_Referee[i + 20] << 8) | Rx_Buf_Referee[i+19];\
						 Referee_date1.powerHeatData.shooterHeat0       = (Rx_Buf_Referee[i + 24] << 8 )| Rx_Buf_Referee[i + 23];\
						 Referee_date1.powerHeatData.shooterHeat1       = (Rx_Buf_Referee[i + 26] << 8 )| Rx_Buf_Referee[i + 25]; 
			...
			//			printf("CmdID=%d	chassisVolt=%f  chassisCurrent=%f  chassisPower=%f  chassisPowerBuffer=%f  shooterHeat0=%d   shooterHeat1=%d\r\n ",\
					...
					{
	//					Referee_date1.RfidDetect.card_Type =  Rx_Buf_Referee[i + 7] ;\
						Referee_date1.RfidDetect.cardidx   =  Rx_Buf_Referee[i + 8] ;
					...
					{
				//	  Referee_date1.GetBuff.buffType     = Rx_Buf_Referee[i + 7];\
						Referee_date1.GetBuff.buffAddition = Rx_Buf_Referee[i + 8];
					...
					{
		//			  Referee_date1.GameRobotPos.x   =  (Rx_Buf_Referee[i + 10] << 24)| (Rx_Buf_Referee[i + 9] << 16)  | (Rx_Buf_Referee[i + 8] << 8)  | Rx_Buf_Referee[i+7];  \
						Referee_date1.GameRobotPos.y   =  (Rx_Buf_Referee[i + 14] << 24)| (Rx_Buf_Referee[i + 13] << 16) | (Rx_Buf_Referee[i + 12] << 8) | Rx_Buf_Referee[i+11]; \
						Referee_date1.GameRobotPos.z   =  (Rx_Buf_Referee[i + 18] << 24)| (Rx_Buf_Referee[i + 17] << 16) | (Rx_Buf_Referee[i + 16] << 8) | Rx_Buf_Referee[i+15]; \
						Referee_date1.GameRobotPos.yaw =  (Rx_Buf_Referee[i + 22] << 24)| (Rx_Buf_Referee[i + 21] << 16) | (Rx_Buf_Referee[i + 20] << 8) | Rx_Buf_Referee[i+19];
