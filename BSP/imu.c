...
void IMU_getValues(volatile float * values) {  
	...
	int i;
	//读取加速度和陀螺仪的当前ADC
    MPU6500_GetMotion6(&accgyroval[0], &accgyroval[1], &accgyroval[2], &accgyroval[3], &accgyroval[4], &accgyroval[5]);
    for(i = 0; i<6; i++) {
      if(i < 3) {
 ...
 void GetPitchYawGxGyGz(MyData *mydata)
{
	...
	mydata->imudata.Gyro_X = mygetqval[3];
	mydata->imudata.Gyro_Y = mygetqval[4];
	mydata->imudata.Gyro_Z = mygetqval[5];
	mydata->imudata.Angle_Yaw += mydata->imudata.Gyro_Z * 0.001f;
	mydata->imudata.Angle_pitch = angle[1];
	mydata->imudata.Angle_Roll = angle[2];
